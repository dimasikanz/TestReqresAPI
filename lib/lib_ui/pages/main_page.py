...
class MainPage(BasePage):
    ...
    def click_button_with_api(self, locator, timeout=None):
        ...
        elem.click()
        if elem != self.wait(timeout).until(EC.element_to_be_clickable(self.locators.DEFAULT_PRESSED_BUTTON_LOCATOR)):
            while start_request_url==current_url:
                current_url = self.find(self.locators.URL_LOCATOR).text
        #Ожидание прокрутки "Спиннера" (Отправка запроса -> получение ответа)
        self.wait(timeout).until(EC.presence_of_element_located(self.locators.HIDDEN_SPINNER_LOCATOR))
