...
class TestGetResource():
    def test_get_page_of_recources(self, api_client):
        response = api_client.request_advanced(method='GET', location=APILocations.RESOURCES_LOCATION)
        page_number_from_response = response.get('page')
        data_from_page = response.get('data')
        assert page_number_from_response==1, (
    ...
    def test_get_certain_recource(self, api_client, setup_valid_user):
        request_resource_id = 2
        response = api_client.request_advanced(method='GET', location=f'{APILocations.RESOURCES_LOCATION}/{request_resource_id}')
        id_from_response = response['data']['id']
        assert id_from_response==request_resource_id, (
