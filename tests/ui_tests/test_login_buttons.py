...
class TestLoginButtons:
    def test_login_successful_button_press_shows_correct_request_and_response(
        self, main_page, api_client
    ):
        main_page.click_button_with_api(
            locator=main_page.locators.LOGIN_SUCCESSFUL_BUTTON_LOCATOR
        )
    ...
    def test_login_unsuccessful_button_press_shows_correct_request_and_response(
        self, main_page, api_client
    ):
        main_page.click_button_with_api(
            locator=main_page.locators.LOGIN_UNSUCCESSFUL_BUTTON_LOCATOR
        )
        ui_request_url = main_page.find(locator=main_page.locators.URL_LOCATOR)
        ui_request_data = eval(
            main_page.find(locator=main_page.locators.OUTPUT_REQUEST_LOCATOR).text
